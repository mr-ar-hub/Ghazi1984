        ...
        <div class="card">
            <div class="card-header">
                <div class="card-title" style="width: 100%;">
                    <h5>Spotted In Ghazi<a href="{{ route('addNewSpotted') }}" class="btn btn-primary btn-sm float-right"><i class="fa fa-plus"></i> Add Spotted</a></h5>
                </div>
            </div>
            <div class="card-body">
                    ...
                    <tbody id="first-body">
                        @foreach ($spotted as $item)
                            <tr class="item-drag" data-id="{{ $item->id }}">
                                ...
                                <td><a href="{{ route('editSpotted', ['id' => $item->id]) }}" style="margin-top: 30px;">{{ $item->name }}</a></td>
                                <td>
                                    @if ($item->SpottedImage)
                                        <img src="{{ asset('storage/' . $item->SpottedImage->image_name) }}" width="100px" height="100px" data-toggle="modal" data-target="#imagePreviewModal" data-src="{{ asset('storage/' . $item->SpottedImage->image_name) }}">
                                    @endif
                                </td>
                                <td>
                                    <form id="status-form-{{ $item->id }}" method="POST" action="{{ route('updateSpottedStatus', ['id' => $item->id]) }}">
                                        @csrf
                                        <label class="switch mt-4">
                                            <input type="checkbox" onChange='submitForm({{ $item->id }});' @if($item->status) checked @endif>
                                            <span class="slider round"></span>
                                        </label>
                                    </form>
                                </td>
                                <td class="text-center">
                                    <a class="btn btn-success btn-sm mt-4" href="{{ route('editSpotted', ['id' => $item->id]) }}"><i class="fa fa-edit"></i></a>
                                    <a class="btn btn-danger btn-sm mt-4" href="{{ route('deleteSpotted', ['id' => $item->id]) }}"><i class="fa fa-trash"></i></a>
                                </td>
                            </tr> 
                        @endforeach
                    </tbody>
...
<script>
    ...
    function updateOrderNo(currentOrderList) {
        $.ajax({
            type: 'POST',
            url: "{{ route('updateSpottedImageOrder') }}",
