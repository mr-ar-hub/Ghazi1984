        ...
        <div class="card">
            <div class="card-header">
                <div class="card-title" style="width: 100%;">
                    <h5>Carousel Images<a href="{{ route('addNewImage') }}" class="btn btn-primary btn-sm float-right"><i class="fa fa-plus"></i> Add Carousel Image</a></h5>
                </div>
            </div>
            <div class="card-body">
                    ...
                    <tbody id="first-body">
                        @foreach ($carousel as $item)
                            <tr class="item-drag" data-id="{{ $item->id }}">
                                ...
                                <td>
                                    <img src="{{ asset('storage/' . $item->image_name) }}" width="100px" height="100px" data-toggle="modal" data-target="#imagePreviewModal" data-src="{{ asset('storage/' . $item->image_name) }}">
                                </td>
                                <td>
                                    <form id="status-form-{{ $item->id }}" action="{{ route('updateCarouselStatus', ['id' => $item->id]) }}" method="POST">
                                        @csrf
                                ...
                                <td class="text-center">
                                    <a class="btn btn-danger btn-sm mt-4" href="{{ route('deletecarouselImage', ['id' => $item->id]) }}"><i class="fa fa-trash"></i></a>
                                </td>
                            </tr>  
                        @endforeach
                    </tbody>
...
<script>
    $(document).ready(function() {
        ...
        function updateOrderNo(currentOrderList) {
            $.ajax({
                type: 'POST',
                url: "{{ route('updateCarouselImageOrder') }}",
