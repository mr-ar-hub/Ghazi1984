        ...
        <div class="card">
            <div class="card-header">
                <div class="card-title" style="width: 100%;">
                    <h5>Blogs<a href="{{ route('addNewBlog') }}" class="btn btn-primary btn-sm float-right"><i class="fa fa-plus"></i> Add Blog</a></h5>
                </div>
            </div>
            <div class="card-body">
                    ...
                    <tbody id="first-body">
                        @foreach ($blog as $item)
                            <tr class="item-drag" data-id="{{ $item->id }}">
                                ...
                                <td><a href="{{ route('editBlog', ['id' => $item->id]) }}" style="margin-top: 30px;">{{ $item->title }}</a></td>
                                <td>{{ $item->auther_name }}</td>
                                <td>{{ $item->blog_date }}</td>
                                <td>
                                    <form id="status-form-{{ $item->id }}" method="POST" action="{{ route('updateBlogStatus', ['id' => $item->id]) }}">
                                        @csrf
                                        <label class="switch">
                                ...
                                <td class="text-center">
                                     <a class="btn btn-info btn-sm" href="{{ route('blogComments', ['id' => $item->id]) }}" data-toggle="tooltip" 
                                        data-placement="top" 
                                        title="View Comments"><i class="fa fa-book"></i></a>
                                    <a class="btn btn-success btn-sm" href="{{ route('editBlog', ['id' => $item->id]) }}"><i class="fa fa-edit"></i></a>
                                    <a class="btn btn-danger btn-sm" href="{{ route('deleteBlog', ['id' => $item->id]) }}"><i class="fa fa-trash"></i></a>
                                </td>
                            </tr> 
                        @endforeach
                    </tbody>
...
<script>
    ...
    function updateOrderNo(currentOrderList) {
        $.ajax({
            type: 'POST',
            url: "{{ route('updateBlogImageOrder') }}",
