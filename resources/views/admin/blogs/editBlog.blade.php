                    ...
                    <div class="card-body">
                        <form action="{{ route('updateBlog', ['id' => $blog->id]) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="title" placeholder="Enter Blog Title" value="{{ $blog->title }}"  class="form-control" />
                            </div>
                            <div class="row">
                                <div class="form-group col-6">
                                    <label>Auther Name</label>
                                        <input type="text" name="auther_name" value="{{ $blog->auther_name }}"  class="form-control" />
                                </div>
                                <div class="form-group col-4">
                                    <label>Blog Date</label>
                                        <input type="date" name="blog_date" value="{{ $blog->blog_date }}"  class="form-control" />
                                </div>
                                <div class="form-group col-2">
                                    <label>Feature</label>
                                    <div>
                                        <input type="checkbox" name="feature" style="width: 20px; height: 20px; margin-top: 6px;" @if($blog->feature) checked @endif>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="editor1" placeholder="Enter description of blog" name="blog_description">{{ $blog->blog_description }}</textarea>
                            </div>
                            <div class="row mt-3">
                            ...
                            <div class="row">
                                <div class="col-sm-12 mt-2">
                                    ...
                                    <a href="{{ route('updateBlog', ['id' => $blog->id]) }}" class="btn btn-danger" style="float: right; padding: 5px 50px;">Cancel</a>
                                </div>
                            </div>
                        </form>
                    </div>
...
<script type="text/javascript">    
    ...
    const dropzone2 = new Dropzone("#blog_image", { 
        ...
        init: function() {
            ...
            @if ($blog->BlogImage)
            var mockFile = { 
                    name: "{{ $blog->BlogImage->image_name }}", 
                    dataURL: "{{ asset('storage/' . $blog->BlogImage->image_name) }}", 
                    meta_data: "{{ $blog->BlogImage->id }}",
                    accepted: true 
                };
                ...
                $("#blog_images").append('<input type="hidden" id="blog_images_val_{{ $blog->BlogImage->id }}" name="blog_images_val[]" value="{{ $blog->BlogImage->id }}">');
            @endif
