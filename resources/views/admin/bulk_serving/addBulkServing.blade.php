                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-title" style="width: 100%;">
                            <h5>Add New Product <a href="{{ route('indexbulkSurvingOrder') }}" class="btn btn-primary btn-sm float-right"><i class="fas fa-arrow-left"></i> Back</a></h5>
                        </div>  
                    </div>
                    <div class="card-body">
                        <form action="{{route('postBulkSurvingOrder')}}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="row mt-3">
                                <div class="form-group col-6">
                                    <label>Customer Name</label>
                                    <input type="text" name="name" placeholder="Enter bulk order serving name" required="" value="{{old('name')}}"  class="form-control" />
                                </div>
                                <div class="form-group col-6">
                                    <label>Company Name</label>
                                    <input type="text" name="company_name" placeholder="Enter company name" required="" value="{{old('company_name')}}"  class="form-control" />
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="editor1">Blog Description</label>
                                <textarea id="editor1" placeholder="Enter description of bulk order serving" name="bluk_serving_description">{{ old('bluk_serving_description') }}</textarea>
                            </div>
                            <div class="row mt-3">
                            ...
                            <div class="row">
                                <div class="col-sm-12 mt-2">
                                    ...
                                    <a href="{{ route('addbulkSurvingOrder') }}" class="btn btn-danger" style="float: right; padding: 5px 50px;">Cancel</a>
                                </div>
                            </div>
                        </form>
                    </div>
...
<script type="text/javascript">    
    ...
    function deletebulkServingImage(id) {
        $.ajax({
            type: "GET",
            url: "{{ route('deletebulkServingImage') }}/" + id,
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
