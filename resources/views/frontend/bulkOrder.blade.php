        ...
        <div class="container">
            <header class="entry-header">
                ...
                <div class="breadcrumbs">
                    <a href="{{ route('index') }}">Home</a> » <span class="current">Bulk Order</span>
                </div>
            </header>
        </div>
    </div>
        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card" style="background-color: none; border: none;">
              ...
              <div class="card-body">
                <form action="{{ route('bulkOrderPost') }}" method="POST" enctype="multipart/form-data">
                  @csrf
                  <div class="row">
                    <div class="form-group col-md-6">
                      <label>Name:</label>
                      <input type="text" required="" value="{{ old('name') }}" name="name" class="form-control" placeholder="Enter name" style=" border: none; border-bottom: 1px solid lightgray; box-shadow: none;outline: none;"/>
                    </div>
                    <div class="form-group col-md-6">
                      <label>Email:</label>
                        <input type="email" required="" value="{{ old('email') }}" name="email" class="form-control" placeholder="Enter email" style=" border: none; border-bottom: 1px solid lightgray; box-shadow: none;outline: none;"/>
                    </div>
                  </div>
                  <div class="row mt-3">
                    <div class="form-group col-md-6">
                      <label>Company Name:</label>
                      <input type="text" required="" value="{{ old('company_name') }}" name="company_name" class="form-control" placeholder="Enter company name" style=" border: none; border-bottom: 1px solid lightgray; box-shadow: none;outline: none;"/>
                    </div>
                    <div class="form-group col-md-6">
                      <label>Whatsapp No:</label>
                        <input type="number" required="" value="{{ old('phone') }}" name="phone" class="form-control" placeholder="Enter whatsapp no." style=" border: none; border-bottom: 1px solid lightgray; box-shadow: none;outline: none;"/>
                    </div>
                  </div>
                  <div class="row mt-3">
                    <div class="form-group col-md-6">
                      <label>Quantity:</label>
                      <input type="number" required="" value="{{ old('quantity') }}" name="quantity" min="1" class="form-control" placeholder="Enter quantity" style=" border: none; border-bottom: 1px solid lightgray; box-shadow: none;outline: none;"/>
                    </div>
                    <div class="form-group col-md-6">
                        <label>Country:</label>
                  ...
                  <div class="form-group mt-3">
                    <label>Address:</label>
                    <input type="text" required="" value="{{ old('address') }}" name="address" class="form-control" placeholder="Enter address" style=" border: none; border-bottom: 1px solid lightgray; box-shadow: none;outline: none;"/>
                  </div>
                  <div class="row mt-3">
                    <div class="form-group col-md-6">
                      <label>Requirements:</label>
                      <textarea id="editor1" placeholder="Enter requirement" name="requirement">{{ old('requirement') }}</textarea>
                    </div>
          ...
          @foreach ($bulkServing as $item)
              <div class="blog-post">
                  <div class="blog-image-wrapper">
                      <img src="{{ asset('storage/' . $item->BulkSurvingImage->image_name) }}" class="img-fluid blog-image"alt="Blog Image 1">
                  </div>
                  <div class="blog-content">
                    <h3 class="blog-title">{{ $item->name }}</h3>
                    <p>{{ $item->company_name }}</p>
                    <p>{!! $item->bluk_serving_description !!}</p>
                  </div>
              </div>
          @endforeach
      </div>
    ...
    <script type="text/javascript">    
      Dropzone.autoDiscover = false;
        ...
        function deleteSampleImage(id) {
            $.ajax({
                type: "GET",
                url: "{{ route('deleteSampleImage') }}/" + id,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
